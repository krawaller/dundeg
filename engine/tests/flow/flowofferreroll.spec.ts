...
test('flow hero reroll choice', t => {
  let result, q, battle: BattleState = {
    heroes: { hero: makeHero('bloodsportBrawler', {luck: 777, attackDice: [7,8], powerDice: [9]}) },
    monsters: { },
  };

  result = <FlowInstruction>flow_offer_reroll(battle, {heroId: 'hero',diceTypes:{attack:2,power:1}});
  ...
  t.deepEqual(
    ...
    <FlowInstruction>['apply', 'reroll', {heroId: 'hero', index: 1, diceType: 'attack'}],
    'correct reroll instruction for second atk die'
  );
  t.deepEqual(
    q.options['pow die with 9'][2][0],
    <FlowInstruction>['apply', 'reroll', {heroId: 'hero', diceType: 'power', index: 0}],
  ...
  result = <FlowInstruction>flow_offer_reroll(battle, {heroId: 'hero',diceTypes:{attack:1}});
  q = <Question>result[2];
  ...
  t.ok(
    !flow_offer_reroll(battle, {heroId: 'hero',diceTypes:{attack:2,defence:2,power:1}}),
