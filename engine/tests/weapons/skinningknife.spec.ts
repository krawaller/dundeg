...
test('skinning knife', t => {
  let result: BattleState, battle: BattleState = {
    heroes: {
      hero: makeHero('bloodsportBrawler',{attackDice:[1,3]},{},{},['skinningKnife'])
    },
    monsters: {
  ...
  let attack:Attack = { using: 'skinningKnife', type: 'meelee', stat: 'AGI' };

  t.deepEqual(
    find_hero_attack_options(battle, {heroId: 'hero'})[items.skinningKnife.actions.skinningKnifeAttack],
    attack,
