...
test('find party stat', t => {
  ...
  let result = find_party_stat(battle,{stat:'CON', reason: '_testReason'});
  t.equal( Object.keys(result.individual).length, 4, 'Only 4 heroes were included' );
  ...
  t.deepEqual( result.ordered, [{value: 9, heroes: ['has9con']}, {value:8, heroes: ['has8con','alsohas8','another8']}] );
