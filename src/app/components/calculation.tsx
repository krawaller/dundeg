...
export const Calculation = withBattle((props) => {
  const rows = props.calculation[0].history.map((entry, n) => (
    <tr key={n}>
      <td>
        <LogLine line={entry[0]} where="calc" />
      </td>
      <td>{entry[1]}</td>
    </tr>
  ))
  return (
    <div className="calctable">
      {props.calculation.length > 1 && (
        <button onClick={(e) => props.backCalculation()}>back</button>
      )}
