...
export function typedConnect<OwnProps, StateProps, DispatchProps>(
  ...
  return function componentImplementation(
    component: React.StatelessComponent<OwnProps & StateProps & DispatchProps>
  ) {
    ...
    )(component) as any as React.StatelessComponent<OwnProps>
  }
}
...
export function withBattle<T>(
  component: React.StatelessComponent<T & stateProps & actions>
) {
  return typedConnect<T, stateProps, any>(
    ...
    (dispatch) => ({
      ...
      showCalculation: (calc, dig) =>
        dispatch(actions.showCalculation(calc, dig)),
      backCalculation: () => dispatch(actions.backCalculation()),
      beginBattle: () => dispatch(actions.beginBattle()),
    })
  )(component)
}
