...
export function flow_select_action(battle: BattleState, {heroId}:SelectActionSpec): FlowInstruction {
  ...
  let attacks = mapValues(find_hero_attack_options(battle, {heroId}),attack => registerAndTarget(heroId,['flow','performHeroAttack',{heroId,attack}],['Who should',{heroRef:heroId},'target?']));
