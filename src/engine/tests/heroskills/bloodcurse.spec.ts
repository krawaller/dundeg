...
test('blood curse hero skill', (t) => {
  let battle: BattleState = {
    heroes: {
      curseLinkedHero: makeHero(
        'bloodsportBrawler',
        { HP: 9, failedDefence: true },
        {},
        { bloodCurse: true }
      ),
    ...
    monsters: {
      cursedMonster: makeMonster('slitherFish', {
        HP: 5,
        target: 'curseLinkedHero',
      }),
      nextMonster: makeMonster('imperialHuntsman', {
        HP: 5,
        target: 'curseLinkedHero',
      }), // ATK 4
      thirdMonster: makeMonster('imperialHuntsman', {
        target: 'curseLinkedHero',
      }), // ATK 4
    },
    log: [],
  ...
  t.ok(
    !find_hero_actions(battle, { heroId: 'curseLinkedHero' })[
      heroSkills.bloodCurse.actions.castBloodCurse
    ],
  ...
  battle = execUntil(battle, [
    ...
    'executeAction',
    { heroId: 'curseLinkedHero' },
  ]) // initiate throw blood curse
  battle = reply(battle, makeRoll) // roll dice, gonna fail because 4+6 > 5
  t.ok(
    !battle.heroes.curseLinkedHero.vars.bloodCurseLink,
    'blood curse was not added since invocation failed'
  )
  ...
  t.equal(
    battle.heroes.curseLinkedHero.vars.bloodCurseLink,
    'cursedMonster',
    'blood curse link set correctly'
  )
  t.equal(
    battle.monsters.cursedMonster.states.bloodCurse,
    'curseLinkedHero',
    'blood curse state added to monster'
  )
  ...
  ]) // will do 4 DMG to hero
  t.equal(battle.heroes.curseLinkedHero.vars.HP, 5, 'hero HP is now 9-4=5')
  t.equal(
    battle.monsters.cursedMonster.vars.HP,
    1,
    'cursed monster took same damage (5-4=1)'
  )
  ...
  battle = execUntil(battle, [
    ...
    'performMonsterAttack',
    { monsterId: 'thirdMonster' },
  ]) // will knock monster out
  t.equal(battle.heroes.curseLinkedHero.vars.HP, 1, 'hero HP is now 5-4=1')
  t.equal(
    battle.monsters.cursedMonster.vars.HP,
    0,
    'monster was knocked out by curse'
  )
  t.ok(
    !battle.monsters.cursedMonster.states.bloodCurse,
    'blood curse state was removed from monster'
  )
  ...
  t.equal(
    battle.heroes.curseLinkedHero.vars.HP,
    0,
    'hero was knocked out as he should have been'
  )
  t.equal(
    battle.monsters.nextMonster.vars.HP,
    4,
    'cursed monster only took 1 HP, since that is what was left for hero'
  )
