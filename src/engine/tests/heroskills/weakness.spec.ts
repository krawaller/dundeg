...
test('the hero weakness skill', (t) => {
  ...
  if (!battle.heroes.hero.vars.target)
    battle = reply(battle, battle.monsters.monster.name) // maybe auto selected since only 1
  ...
  t.ok(
    !result.monsters.monster.states.weakness,
    'Weakness wasnt applied since invocation failed'
  )
  ...
  t.equal(
    result.monsters.monster.states.weakness,
    'hero',
    'Weakness state was correctly set'
  )
  ...
  result = execUntil(result, <FlowPerformHeroAttack>[
    'flow',
    'performHeroAttack',
    { heroId: 'hero', attack: { stat: 'STR' } },
  ])
  t.equal(
