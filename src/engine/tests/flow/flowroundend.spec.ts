...
test('flow round end', (t) => {
  ...
  battle.heroes.hero2.vars.action = [
    'flow',
    'throwShrapnelBomb',
    { heroId: 'hero2' },
  ]
  ...
  result = execTo(
    battle,
    ['flow', 'roundEnd', {}],
    ['winGame', 'loseGame', 'newRound']
  )
  t.ok(!result.heroes.hero.vars.hasActed, 'hasActed was removed from heroes')
  t.ok(
    !result.heroes.hero.vars.usedPowerDice,
    'hasUsedPowForDefence was removed from heroes'
  )
  t.ok(
    !result.heroes.hero.vars.failedDefence,
    'failedDefence was removed from heroes'
  )
  ...
  t.ok(
    !result.monsters.monster.vars.hasAttacked,
    'hasAttacked was removed from monsters'
  )
  ...
  t.equal(
    result.stack[0][1],
    'loseGame',
    'we lost the game since no active heroes remain'
  )
  ...
  t.equal(
    result.stack[0][1],
    'winGame',
    'we won the game since all monsters are down'
  )
