...
} from '../../src/flow/flow_monster_target_choice'
...
test('flow monster target choice', (t) => {
  ...
  const q = <Question>result[2]
  t.deepEqual(
    Object.keys(q.options),
    ['Bloodsport Brawler', 'Soldier of Fortune'],
    'we got correct options'
  )

  battle.heroes.butcher = makeHero('infamousButcher') // MRL 6
  result = <FlowTarget>(
    flow_monster_target_choice(battle, { monsterId: 'monster' })
  )
  t.equal(result[0], 'apply')
  t.equal(
    result[1],
    'monsterTargetChoice',
    'we got a monsterTargetChoice instruction'
  )
