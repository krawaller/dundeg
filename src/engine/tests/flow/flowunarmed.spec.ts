...
test('unarmed hero attacks', (t) => {
  ...
  let battle: BattleState = {
    heroes: {
      hero: makeHero('infamousButcher', { target: 'monster', HP: startingHP }),
    }, // AGI 8, STR 8
    monsters: { monster: makeMonster('slimeCorpse', { target: 'hero' }) }, // ATK 4, ARM 1, HP 8
    log: [],
  }
  const blueprint = monsters.slimeCorpse
  let result: BattleState
  const DMG = blueprint.stats.ATK

  result = execUntil(battle, ['flow', 'selectAction', { heroId: 'hero' }])
  t.ok(
    result.question.options[misc.basicActions.unarmedAGI],
    'Hero gets unarmed AGI attack'
  )
  t.ok(
    result.question.options[misc.basicActions.unarmedSTR],
    'Hero gets unarmed STR attack'
  )
  ...
  result = execUntil(battle, <FlowInitiateMonsterAttack>[
    'flow',
