...
test('flow battle dice', (t) => {
  ...
  t.ok(
    !result.question,
    'no question was posed since we dont need to roll anything'
  )
  ...
  t.deepEqual(
    result.heroes.hero.vars.defenceDice,
    [3, 4],
    'we rolled defence dice since monster targetting us'
  )
  t.equal(result.heroes.hero.vars.powerDice[0], 6, 'we rolled power die')
  t.ok(
    !result.heroes.hero.vars.attackDice,
    'we did not roll attack die since no regular attack'
  )
  ...
  t.deepEqual(
    result.heroes.hero.vars.attackDice,
    [6, 3],
    'we rolled attack dice since were doing regular attack'
  )
  t.equal(result.heroes.hero.vars.powerDice[0], 1, 'we rolled power die')
  t.ok(
    !result.heroes.hero.vars.defenceDice,
    'we did not roll defence die since no one targets us'
  )
  ...
  t.deepEqual(
    result.heroes.hero.vars.defenceDice,
    [4, 5],
    'we did rolled defence since monster targetting us'
  )
  t.equal(result.heroes.hero.vars.powerDice[0], 6, 'we rolled power die')
