...
export function logMessageContains(msg: LogMessageLine, str: string) {
  return msg.find(
    (part) => !!((<string>part).match && (<string>part).match(str))
  )
}
...
export function lastLogHasStr(battle: BattleState, str: string) {
  ...
  const lastMsg = battle.log[battle.log.length - 1]
  return lastMsg.line.find(
    (part) => !!((<string>part).match && (<string>part).match(str))
  )
}

export function makeMonster(
  monsterName: MonsterBase,
  vars?: MonsterVars,
