...
export function apply_action_selection(
  battle: BattleState,
  { heroId, action }: ActionSelectionSpec
): BattleState {
  ...
  if (
    action[1] === 'performHeroAttack' &&
    (<PerformHeroAttackSpec>action[2]).attack.type === 'unarmed'
  ) {
    ...
    ret = addLog(
      ...
      [
        'Because',
        { heroRef: heroId },
        'uses an unarmed attack, POW will count as halved this round',
      ],
      'info'
    )
  }
