...
export function apply_wounds_to_monster(
  battle: BattleState,
  { monsterId, heroId, wounds, attack }: WoundMonsterSpec
): BattleState {
  const ret = deepCopy(battle)
  const target = ret.monsters[monsterId]
  const dealt = Math.min(target.vars.HP, wounds.value)
