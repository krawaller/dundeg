...
export function apply_defence_outcome(
  battle: BattleState,
  { heroId }: DefenceOutcomeSpec
): BattleState {
  ...
  if (chargingMonsters.length) {
    ...
    const agi = calculate_hero_stat(ret, {
      heroId,
      stat: 'AGI',
      reason: 'defence',
    })
    const rolled = hero.vars.defenceDice[0] + hero.vars.defenceDice[1]
    if (rolled > agi.value) {
      ...
      ret = apply_log(ret, {
        ...
        line: [
          ...
          'has AGI',
          agi,
          'but rolled ' + rolled + ' so failed defence',
        ],
      })
    } else {
      apply_log(ret, {
        ...
        line: [
          ...
          'has AGI',
          agi,
          'and rolled ' + rolled + ' so will defend',
        ],
      })
    }
    return ret
