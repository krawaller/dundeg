...
export function apply_dice_roll(
  battle: BattleState,
  { heroId, diceTypes }: DiceRollSpec
): BattleState {
  ...
  if ((temp = diceTypes['attack'])) {
    ...
    while (temp) {
      ...
      msg.push(
        'ATK{' + hero.vars.attackDice[hero.vars.attackDice.length - 1] + '}'
      )
      temp--
    }
  }
  if ((temp = diceTypes['defence'])) {
    ...
    while (temp) {
      ...
      msg.push(
        'DEF{' + hero.vars.defenceDice[hero.vars.defenceDice.length - 1] + '}'
      )
      temp--
    }
  }
  if ((temp = diceTypes['power'])) {
    ...
    while (temp) {
      ...
      msg.push(
        'POW{' + hero.vars.powerDice[hero.vars.powerDice.length - 1] + '}'
      )
      temp--
    }
  }
  ret = apply_log(ret, {
    line: [{ heroRef: heroId }, 'rolled'].concat(msg),
    type: 'action',
  })
  return ret
}
