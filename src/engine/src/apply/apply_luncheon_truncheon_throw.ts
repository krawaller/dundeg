...
export function apply_luncheon_truncheon_throw(
  battle: BattleState,
  { heroId }: LuncheonTruncheonThrowSpec
): BattleState {
  const ret = deepCopy(battle)
  const hero = ret.heroes[heroId]
  addLog(ret, [
    { heroRef: heroId },
    'throws the Luncheon Truncheon amidst the enemies!',
  ])
  ...
  find_monsters(ret, {}).forEach((monsterId) => {
    ...
    if (
      !monster.states.dazed &&
      blueprint.value === 1 &&
      (blueprint.traits.goblin || blueprint.traits.filth)
    ) {
      monster.states.dazed = true
      addLog(ret, [
        { monsterRef: monsterId },
        'is dazed by the Luncheon Truncheon from',
        { heroRef: heroId },
      ])
    }
  })
