...
export const find_hero_attack_options = (
  battle: BattleState,
  { heroId }: InstrJustHeroId
) => {
  const ret: AttackOptions = {
  ...
  if (hero.items.nastyCleaver) {
    ret[items.nastyCleaver.actions.nastyCleaverAttack] = {
      type: 'meelee',
      stat: 'STR',
      using: 'nastyCleaver',
    }
  ...
  if (hero.items.skinningKnife) {
    ret[items.skinningKnife.actions.skinningKnifeAttack] = {
      type: 'meelee',
      stat: 'AGI',
      using: 'skinningKnife',
    }
  ...
  if (hero.items.luncheonTruncheon) {
    ret[items.luncheonTruncheon.actions.luncheonTruncheonAttack] = {
      type: 'meelee',
      stat: 'STR',
      using: 'luncheonTruncheon',
    }
  }
