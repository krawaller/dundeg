...
interface CalculateHeroStatInstr {
  heroId: string
  stat: HeroStatName
  reason: StatCheckReason
}

export function calculate_hero_stat(
  battle: BattleState,
  { heroId, stat, reason }: CalculateHeroStatInstr
): CalculationResult {
  const hero = battle.heroes[heroId]
  const blueprint = heroes[hero.blueprint]
  let ret = newCalc(
    'Hero ' + stat,
    blueprint.name + ' base ' + stat,
    blueprint.stats[stat]
  )
  ...
  if (
    reason === 'ambush' &&
    find_heroes(battle).filter((id) => battle.heroes[id].skills.sixthSense)
      .length
  ) {
    ret = addCalcStep(
      ret,
      'Friend with Sixth Sense gives +1 VS Ambush',
      (n) => n + 1
    )
  }
