...
interface CalculateMonsterAttackInstr {
  monsterId: string
}

export function calculate_monster_attack(
  battle: BattleState,
  instr: CalculateMonsterAttackInstr
): CalculationResult {
  const monster = battle.monsters[instr.monsterId]
  const blueprint = monsters[monster.blueprint]
  const hero = monster.vars.target && battle.heroes[monster.vars.target]

  let ret = newCalc(
    'Monster ATK',
    blueprint.name + ' base ATK',
    blueprint.stats.ATK
  )
  ...
  if (blueprint.skills.horde) {
    const others = find_monsters(battle, {}).reduce((list, otherId) => {
      const other = battle.monsters[otherId]
      if (
        otherId !== instr.monsterId &&
        monsters[other.blueprint].traits[blueprint.skills.horde]
      ) {
        list.push(otherId)
      }
      return list
  ...
  if (monster.states.dazed) {
    ret = addCalcStep(ret, 'Dazed means halved ATK rounding down', (n) =>
      Math.floor(n / 2)
    )
  }
