...
export function flow_next_player(
  battle: BattleState,
  spec: any
): FlowInstruction {
  ...
  if (party.ordered.length === 0 || find_monsters(battle, {}).length === 0) {
    ...
    const nextId = party.ordered[0].heroes[0]
    return [
      'flow',
      'all',
      [
    ...
    party.ordered[0].heroes.forEach((heroId) => {
      ...
      opts[heroes[battle.heroes[heroId].blueprint].name] = <FlowInstruction>[
        'flow',
        'playerRound',
        { heroId },
      ]
